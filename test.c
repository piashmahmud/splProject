...
char keywords[11][20]= {"VarInt","VarChar","VarNum","start","stop","print","get","give","loop","array","string"};

void fileReader()
{
    int i,ch;

    FILE *inputFile = fopen("input_file1.txt","r"); //reading all the character
...
void file_write()
{
    ...
    while(1)
    {
        ...
        if(l==5 && (s[0]='s' && s[1]=='t' && s[2]=='a' && s[3]=='r' && s[4]=='t'))
        {
            //File_Open
            fPtr = fopen("input_file1.txt", "w");
            break;
        }
    ...
    while(1)
    {
        ...
        if(l==5 &&(s[0]=='s' && s[1]=='t' && s[2]=='o' && s[3]=='p'))
        {
...
void variableTranslator()
{
    ...
    while(1)
    {
                 ...
                 stringMatching(inIndx,5) )
        {
            ...
            outputFileString[inIndx+2+gap]='u';
            outputFileString[inIndx+3+gap]='t';
            outputFileString[inIndx+4+gap]='<';
            outputFileString[inIndx+5+gap]='<';
            gap+=1;
            inIndx = dividers[divIndx+1].position;
            divIndx++;
        }
           ...
           stringMatching(inIndx,8) )
        {
            ...
            divIndx++;
                 ...
                 stringMatching(inIndx,8) )
        {
            outputFileString[inIndx+gap]='w';
            outputFileString[inIndx+1+gap]='h';
            outputFileString[inIndx+2+gap]='i';
            outputFileString[inIndx+3+gap]='l';
            outputFileString[inIndx+4+gap]='e';
            gap+=1;
            inIndx = dividers[divIndx+1].position;
            divIndx++;
        }
        ...
        {
            outputFileString[inIndx+gap]='c';
            outputFileString[inIndx+1+gap]='i';
            outputFileString[inIndx+2+gap]='n';
            outputFileString[inIndx+3+gap]='>';
            outputFileString[inIndx+4+gap]='>';
            gap+=2;
            inIndx = dividers[divIndx+1].position;
            divIndx++;
        ...
        else
        {
            ...
            inIndx++;
            if(inIndx-1 == dividers[divIndx+1].position)
                divIndx++;
            if(inputFileString[inIndx-1]=='\0')
                break;
        }
    }
}
...
int main()
{
    ...
    if(q==1)
    {
        ...
        char head[]="#include<iostream>\n\nusing namespace std;\n\n";
        fprintf(outputFile,"%s",head);
        ...
        system("g++ -o output output_file.cpp");
        system("output.exe");
    }
